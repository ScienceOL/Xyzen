...
import { useToggleSkillLike } from "@/hooks/useSkillMarketplace";
import type { SkillMarketplaceListing } from "@/service/skillMarketplaceService";

interface SkillListingCardProps {
  ...
  if (compact) {
    return (
      <div
        ...
        onClick={onClick}
      >
        <div className="absolute inset-0 bg-linear-to-br from-emerald-500/5 via-teal-500/5 to-cyan-500/5 opacity-0 transition-opacity duration-300 group-hover:opacity-100" />

        <div className="relative flex flex-col space-y-1 p-4">
          <div className="flex items-start justify-between">
            <div className="flex min-w-0 items-center gap-2.5">
              <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-linear-to-br from-emerald-500 via-teal-500 to-cyan-500 text-base font-bold text-white shadow-lg">
              ...
              <div className="flex min-w-0 flex-col">
                <div className="flex items-center gap-1.5">
                  ...
                  {listing.scope === "official" && (
                    <span className="inline-flex shrink-0 items-center rounded-full bg-emerald-100 px-1.5 py-0.5 text-[10px] font-semibold text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-300">
                ...
                <p className="line-clamp-1 text-xs text-neutral-500 dark:text-neutral-400">
                  {listing.description ||
                    t("skillMarketplace.card.noDescription")}
                </p>
              </div>
            </div>
            <button
        ...
        <div className="absolute bottom-0 left-0 right-0 h-1 bg-linear-to-r from-emerald-500 via-teal-500 to-cyan-500 opacity-0 transition-opacity duration-300 group-hover:opacity-100" />
      </div>
    );
  }
      ...
      <div className="relative flex flex-col space-y-1.5 p-6 pb-4">
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-3">
            <div className="flex h-14 w-14 items-center justify-center rounded-lg bg-linear-to-br from-emerald-500 via-teal-500 to-cyan-500 text-xl font-bold text-white shadow-lg">
              {listing.name.charAt(0).toUpperCase()}
            </div>
            <div className="flex flex-col">
              <div className="flex items-center gap-1.5">
                ...
                {listing.scope === "official" && (
                  <span className="inline-flex shrink-0 items-center rounded-full bg-emerald-100 px-2 py-0.5 text-xs font-semibold text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-300">
                    {t("skillMarketplace.badge.official")}
                  </span>
                )}
              </div>
              <p className="flex items-center gap-1 text-xs text-neutral-500 dark:text-neutral-400">
                {listing.scope === "official" ? (
                  t("skillMarketplace.card.byOfficial")
                ) : (
                  <>
        ...
        <p className="mt-3 line-clamp-2 text-sm leading-relaxed text-neutral-600 dark:text-neutral-400">
          {listing.description || t("skillMarketplace.card.noDescription")}
        </p>
      </div>
          ...
          <div className="mb-4 flex flex-wrap gap-1.5">
            ...
            {listing.tags.length > 3 && (
              <span className="inline-flex items-center rounded-full border border-neutral-200 bg-neutral-50 px-2.5 py-1 text-xs font-medium text-neutral-700 dark:border-neutral-700 dark:bg-neutral-900 dark:text-neutral-300">
                {t("skillMarketplace.card.tagsMore", {
