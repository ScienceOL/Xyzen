...
export function ConsumptionAnalytics({ onClose }: ConsumptionAnalyticsProps) {
  ...
  const rowRefCallback = useCallback(
    (date: string) => (el: HTMLTableRowElement | null) => {
  ...
  // Tier donut data derived per metric (exclude "unknown" tier)
  const knownTierEntries = useMemo(
    () => Object.entries(displayData.byTier).filter(([k]) => k !== "unknown"),
    [displayData],
  );
  const tierAmountEntries = useMemo(
    () =>
      knownTierEntries.map(([k, v]) => [k, v.total_amount] as [string, number]),
    [knownTierEntries],
  );
  const tierTokenEntries = useMemo(
    () =>
      knownTierEntries.map(([k, v]) => [k, v.total_tokens] as [string, number]),
    [knownTierEntries],
  );
  const tierInputTokenEntries = useMemo(
    () =>
      knownTierEntries.map(([k, v]) => [k, v.input_tokens] as [string, number]),
    [knownTierEntries],
  );
  const tierOutputTokenEntries = useMemo(
    () =>
      knownTierEntries.map(
        ([k, v]) => [k, v.output_tokens] as [string, number],
      ),
    [knownTierEntries],
  );
  const tierCountEntries = useMemo(
    () =>
      knownTierEntries.map(([k, v]) => [k, v.record_count] as [string, number]),
    [knownTierEntries],
  );
  const tierToolCallEntries = useMemo(
    () =>
      knownTierEntries.map(
  ...
  const amountDonutOption = useMemo(
    () => makeTierDonut(tierAmountEntries, isDark, donutEmptyLabel),
    [tierAmountEntries, isDark, donutEmptyLabel],
  );
  const tokenDonutOption = useMemo(
    () => makeTierDonut(tierTokenEntries, isDark, donutEmptyLabel),
    [tierTokenEntries, isDark, donutEmptyLabel],
  );
  const inputTokenDonutOption = useMemo(
    () => makeTierDonut(tierInputTokenEntries, isDark, donutEmptyLabel),
    [tierInputTokenEntries, isDark, donutEmptyLabel],
  );
  const outputTokenDonutOption = useMemo(
    () => makeTierDonut(tierOutputTokenEntries, isDark, donutEmptyLabel),
    [tierOutputTokenEntries, isDark, donutEmptyLabel],
  );
  const countDonutOption = useMemo(
    () => makeTierDonut(tierCountEntries, isDark, donutEmptyLabel),
    [tierCountEntries, isDark, donutEmptyLabel],
  );
  const toolCallDonutOption = useMemo(
    () => makeTierDonut(tierToolCallEntries, isDark, donutEmptyLabel),
    [tierToolCallEntries, isDark, donutEmptyLabel],
  );
  ...
  }, [data, isDark, yearToShow, t]);

  // Daily table — only days with activity, descending order, client-side paged
        ...
        <div className="flex items-center gap-2">
          {onClose && (
            <Button
              ...
              size="sm"
              onClick={onClose}
              className="h-8 gap-1.5 border-neutral-200/70 bg-white/70 text-neutral-600 hover:bg-white dark:border-neutral-700/60 dark:bg-neutral-900/60 dark:text-neutral-300 dark:hover:bg-neutral-900"
            >
              <XMarkIcon className="h-3.5 w-3.5" />
              {t("app.consumption.close")}
            </Button>
          )}
          <Button
            variant="ghost"
          ...
          <div className="grid grid-cols-2 gap-3">
            <Card className="backdrop-blur-md bg-white/70 dark:bg-neutral-900/70 border-white/20 dark:border-neutral-700/30 shadow-lg">
              <CardContent className="p-3">
                ...
                <div className="mt-1 text-xl font-bold text-neutral-900 dark:text-white text-center">
                  {isLoading ? (
                ...
                {isLoading ? (
                  <div className="h-20 mt-1 animate-pulse rounded bg-neutral-100 dark:bg-neutral-800" />
                ) : (
                  <ReactECharts
                      ...
                      {pagedRows.length === 0 ? (
                        ...
                        pagedRows.map((d) => {
                          ...
                          const tierEntries = Object.entries(d.by_tier)
                            .filter(
                              ([k, v]) => k !== "unknown" && v.record_count > 0,
                            )
                            .sort(
