...
export default function PublishAgentModal({
  ...
  // Sync tags when modal opens with new initial values
  useEffect(() => {
  ...
  return (
    <SheetModal isOpen={open} onClose={() => onOpenChange(false)} size="xl">
      {/* Header */}
      ...
      <div className="custom-scrollbar flex-1 overflow-y-auto">
        <div className="px-5 py-5">
          {/* --- Shared field blocks (rendered once, referenced in both layouts) --- */}
          {(() => {
            const infoAlert = (
            ...
            const commitMessageField = (
              ...
              >
                <Textarea
                  id="commit-message"
                  placeholder={t(
                    "marketplace.publish.commitMessage.placeholder",
                  )}
                  value={commitMessage}
            ...
            const tagsField = (
              <FieldGroup
                label={t("marketplace.publish.tags.label")}
                hint={t("marketplace.publish.tags.hint")}
              >
                <div className="flex flex-wrap items-center gap-1.5 rounded-sm border border-neutral-200 bg-white px-2.5 py-2 transition-colors focus-within:border-indigo-400 focus-within:ring-2 focus-within:ring-indigo-500/20 dark:border-neutral-700 dark:bg-neutral-800">
                  {tags.map((tag) => (
                    <span
                      key={tag}
                      className="inline-flex items-center gap-1 rounded-full bg-indigo-50/80 px-2.5 py-0.5 text-xs font-medium text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-300"
                    >
                      {tag}
                      <button
                        type="button"
                        onClick={() => removeTag(tag)}
                        className="ml-0.5 rounded-full p-0.5 transition-colors hover:bg-indigo-200/60 dark:hover:bg-indigo-800/40"
                      >
                  ...
                  <input
                    ...
                    placeholder={
                      tags.length === 0
                        ? t("marketplace.publish.tags.placeholder")
                        : ""
                    }
                    className="min-w-[80px] flex-1 border-none bg-transparent py-0.5 text-[13px] text-neutral-700 outline-none placeholder:text-neutral-400 dark:text-neutral-300 dark:placeholder:text-neutral-500"
                  />
                </div>
              </FieldGroup>
            );

            const knowledgeBaseField = (
              <FieldGroup
            ...
            return (
              <div className="space-y-6">
                ...
                {/* Desktop: two columns */}
                <div className="hidden md:grid md:grid-cols-[55%_45%]">
                  <div className="space-y-6 pr-5 border-r border-neutral-200/60 dark:border-neutral-800/60">
                    {infoAlert}
                    {commitMessageField}
                    {readmeField}
                    {tagsField}
