...
export default function XyzenChat() {
  ...
  const [showHistory, setShowHistory] = useState(() => {
    const savedHistoryState = localStorage.getItem("chatHistoryPinned");
  ...
  const handleToggleHistory = () => {
    ...
    localStorage.setItem("chatHistoryPinned", newState.toString());
  };

  const handleCloseHistory = () => {
    setShowHistory(false);
    localStorage.setItem("chatHistoryPinned", "false");
  };
  ...
  return (
    <div className={`${showHistory ? "flex" : "flex flex-col"} h-full`}>
      {/* Main Chat Content Wrapper */}
      <div className={`${showHistory ? "flex-1" : ""} flex flex-col h-full`}>
        {!activeChatChannel ? (
          <>
            <EmptyChat />
