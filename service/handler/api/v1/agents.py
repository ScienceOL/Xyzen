...
async def create_agent(*, session: AsyncSession = Depends(get_session), agent_in: AgentCreate) -> Agent:
    ...
    if not provider:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail=f"Provider with id {agent_in.provider_id} not found"
        )
...
async def list_agents(*, session: AsyncSession = Depends(get_session), skip: int = 0, limit: int = 100) -> list[Agent]:
    """
...
async def update_agent(
    *, session: AsyncSession = Depends(get_session), agent_id: UUID, agent_update: AgentUpdate
) -> Agent:
    """
...
@router.post(
    "/sessions/", response_model=SessionRead, status_code=status.HTTP_201_CREATED, summary="为Agent创建新会话"
)
async def create_session_for_agent(
    *, session: AsyncSession = Depends(get_session), session_in: SessionCreate
) -> Session:
    """
    ...
    if not agent:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail=f"Agent with id {session_in.agent_id} not found"
        )
