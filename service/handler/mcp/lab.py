...
async def list_laboratory_devices() -> dict:
    """
    Lists laboratory devices by calling the internal lab API.
    Authentication is handled automatically on the server.
    
    Returns:
        Dictionary containing lab name and list of devices, or an error.
    """
    try:
        api_secret = configs.LAB.SecretKey
        if not api_secret:
            raise ValueError("API SecretKey is not configured on the server.")
            
        url = f"{configs.LAB.ApiBaseUrl}/api/environment/lab/mcp"
...
async def list_device_actions(device_id: str) -> dict:
    ...
    try:
        ...
        url = f"{configs.LAB.ApiBaseUrl}/api/environment/lab/mcp/actions/"
...
@lab_mcp.tool
async def perform_device_action(device_id: str, action_type: str, action: str, command: str) -> dict:
    """
    ...
    try:
        ...
        url = f"{configs.LAB.ApiBaseUrl}/api/environment/lab/mcp/execute/"
        params = {"secret_key": api_secret, "device_id": device_id}
...
async def get_device_status(device_id: str) -> dict:
    ...
    try:
        ...
        url = f"{configs.LAB.ApiBaseUrl}/api/environment/lab/mcp/device_status/"
        params = {"secret_key": api_secret, "device_id": device_id}
